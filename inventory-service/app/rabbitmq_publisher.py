...
Publishes StockReserved events to the stock_reserved queue
"""
...
def publish_stock_reserved(event_data):
    """
    Publish a StockReserved event to RabbitMQ
    Adds timestamp to event if not already present
    
    Args:
        event_data (dict): Stock reservation event data to publish
    """
    try:
        ...
        print(f"[PUBLISHER] StockReserved event published to {STOCK_RESERVED_QUEUE}")
